    ...
    public CameraManager(Context context) {
        synchronized(mLock) {
            ...
            if (context.getResources().
                ...
                if (DEBUG) {
                    Log.v(TAG, "Use wakelock for flashlight enabled");
                }
                ...
                TorchCallback torchCallback = new CameraManager.TorchCallback() {
                    ...
                    public void onTorchModeUnavailable(String cameraId) {
                        if (mFlashlightWakeLock.isHeld()) {
                            if (DEBUG) {
                                Log.v(TAG, String.format("TorchCallback unavailable: Release wakelock for flashlight cameraId=%s", cameraId));
                            }
                    ...
                    public void onTorchModeChanged(String cameraId, boolean enabled) {
                        if (enabled) {
                            if (!mFlashlightWakeLock.isHeld()) {
                                if (DEBUG) {
                                    Log.v(TAG, String.format("TorchCallback mode changed to enabled: Acquire wakelock for flashlight cameraId=%s", cameraId));
                                }
                            ...
                            if (mFlashlightWakeLock.isHeld()) {
                                if (DEBUG) {
                                    Log.v(TAG, String.format("TorchCallback mode changed to disabled: Release wakelock for flashlight cameraId=%s", cameraId));
                                }
                                mFlashlightWakeLock.release();
                            }
                        }
