...
class LockSettingsShellCommand extends ShellCommand {
    ...
    private void runSetPattern() throws RemoteException {
        ...
        mLockPatternUtils.saveLockPattern(stringToPattern(mNew, lpu.getLockPatternSize(mCurrentUserId)), mOld, mCurrentUserId);
        ...
        if (havePassword || havePattern) {
            ...
            try {
                ...
                if (havePassword) {
                    ...
                    LockPatternUtils lpu = new LockPatternUtils(mContext);
                    result = mLockPatternUtils.checkPattern(stringToPattern(mOld, lpu.getLockPatternSize(mCurrentUserId)), mCurrentUserId);
                }
                if (!result) {
                    if (!mLockPatternUtils.isManagedProfileWithUnifiedChallenge(mCurrentUserId)) {
