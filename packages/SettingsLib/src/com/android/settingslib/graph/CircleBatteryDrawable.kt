...
class CircleBatteryDrawable(private val mContext: Context, frameColor: Int) : Drawable() {
    ...
    private var mPowerSaveAsColorError = true
    ...
    private fun setPowerSaveAsColorError(asError: Boolean) {
    ...
    private fun batteryColorForLevel(level: Int) =
        if (charging || powerSaveEnabled && mPowerSaveAsColorError) {
            mChargeColor
        } else {
            getColorForLevel(
                level
    ...
    override fun draw(c: Canvas) {
        ...
        mFrame[strokeWidth / 2.0f + mPadding.left, strokeWidth / 2.0f, circleSize - strokeWidth / 2.0f + mPadding.left] =
