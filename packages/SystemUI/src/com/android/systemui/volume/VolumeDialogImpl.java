...
public class VolumeDialogImpl implements VolumeDialog,
    ...
    private void initDialog() {
        ...
        // Expanding a View to the left causes a janky relayout animation, avoid that
        // by reserving the space. The touchable region is not affected by this.
        mWindow.setLayout(!mVolumePanelOnLeft ? MATCH_PARENT : WRAP_CONTENT, WRAP_CONTENT);
        ...
        if (mRingerAndDrawerContainer != null) {
            ...
            mRingerAndDrawerContainer.post(() -> {
                ...
                if (ringerAndDrawerBg != null && ringerAndDrawerBg.getNumberOfLayers() > 0) {
                    ...
                    // Rows need to be updated after mRingerAndDrawerContainerBackground is configured
    ...
    private void updateRowsH(final VolumeRow activeRow, boolean animate) {
        ...
        for (final VolumeRow row : mRows) {
            ...
            Log.d(TAG, "mRingerAndDrawerContainerBackground != null: " + (mRingerAndDrawerContainerBackground != null));
        ...
        if (outmostVisibleRowIndex > -1 && outmostVisibleRowIndex < Short.MAX_VALUE) {
            ...
            for (final VolumeRow row : mRows) {
                ...
                Log.d(TAG, "going through rows, shouldBeVisible: " + shouldBeVisible + ", expandable: " + isExpandableRow(row));

                if (shouldBeVisible) {
                    visibleRowsCount++;
                } else if (isExpandableRow(row)) {
                    Log.d(TAG, "incrementing hidden rows count");
                    hiddenRowsCount++;
                }

                // Only move rows that are either expandable or visible and not the default stream
                if ((!isExpandableRow(row) && !shouldBeVisible) || row.defaultStream ) {
                    Log.d(TAG, "not a row which we want to animate. defaultstream: " + row.defaultStream);
                    continue;
                }
                ...
                float translationX = getTranslationForPanelLocation()
                        * hiddenRowsCount * rowWidth;

                Log.d(TAG, "set translationX variable to: " + translationX);
                ...
                if (animate) {
                    ...
                    if (mExpanded && row.view.getTranslationX() == 0f) {
                        Log.d(TAG, "prevent no animation, setting translationX to: " + translationX);
                        row.view.setTranslationX(getTranslationForPanelLocation()
                                * visibleRowsCount * rowWidth);
                    }

                    // The elevation should fall from the outmost row to the inner rows, so that
                    ...
                    row.view.animate()
                            ...
                            .setListener(new AnimatorListenerAdapter() {
                                ...
                                public void onAnimationEnd(Animator animation) {
                                    ...
                                    mAnimatingRows--;
                                    if (mAnimatingRows == 0) {
                                        Log.d(TAG, "final end of animation. Should only appear once");
                                        // Restore the elevation and background
                                        lastVisibleChild.setElevation(0);
                                        lastVisibleChild.setBackgroundColor(Color.TRANSPARENT);
                                        // Set the active stream to ensure the volume keys change the
                                        // volume of the tinted row. The tint was set before already,
                                        // but setting the active row cancels ongoing animations.
