...
public class VolumeDialogImpl implements VolumeDialog,
    ...
    private void addRow(int stream, int iconRes, int iconMuteRes, boolean important,
        ...
        mDialogRowsView.addView(mVolumePanelOnLeft ? row.view : row.view, 0);
        mRows.add(row);
    }
    ...
    public void initSettingsH() {
        ...
        if (mExpandRows != null) {
            mExpandRows.setOnClickListener(v -> {
                // On click, invert expansion states.
                Util.setVisOrGone(findRow(AudioManager.STREAM_RING).view, !mExpanded);
                Util.setVisOrGone(findRow(AudioManager.STREAM_ALARM).view, !mExpanded);
                if (!isNotificationVolumeLinked()) {
                    // Show the notification stream when unlinked
                    Util.setVisOrGone(findRow(AudioManager.STREAM_NOTIFICATION).view,
                            !mExpanded);
                }
                // Ally stream is always visible
                Util.setVisOrGone(findRow(mAllyStream).view, true);
