...
public class NetworkTraffic extends TextView {
    ...
    private View.OnSystemUiVisibilityChangeListener mSystemUiVisibilityChangeListener =
        ...
        public void onSystemUiVisibilityChange(int visibility) {
            ...
            final boolean isVisible =
                    (visibility & (SYSTEM_UI_FLAG_FULLSCREEN | SYSTEM_UI_FLAG_LOW_PROFILE)) == 0;
            if (mVisible != isVisible) {
                mVisible = isVisible;
                mTrafficHandler.sendEmptyMessage(MESSAGE_TYPE_UPDATE_VIEW);
            }
    ...
    private Handler mTrafficHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            long txBytes = TrafficStats.getTotalTxBytes() - mTotalTxBytes;
            long rxBytes = TrafficStats.getTotalRxBytes() - mTotalRxBytes;
            mTotalRxBytes += rxBytes;
            mTotalTxBytes += txBytes;

            if (shouldHide(txBytes, rxBytes, timeDelta)) {
                setText("");
                setVisibility(GONE);
                ...
                if (shouldShowUpstream()) {
                    output.append(formatOutput(timeDelta, txBytes));
                }
                ...
                if (shouldShowDownstream()) {
                    output.append(formatOutput(timeDelta, rxBytes));
                }
            ...
            // Post delayed message to refresh every REFRESH_INTERVAL
            if (mVisible) {
        ...
        private String formatOutput(long timeDelta, long bytes) {
            final float kbps = 8 * (float) bytes / (timeDelta / 1000f) / 1000f;
            final float value;
            final String unit;
            ...
            return mContext.getString(R.string.network_traffic_format_with_unit, value, unit);
        }

        private boolean shouldHide(long txBytes, long rxBytes, long timeDelta) {
            if (!mAutoHide) {
                return false;
            }
            long speedTxKB = (long) (txBytes / (timeDelta / 1000f)) / 1024;
            long speedRxKB = (long) (rxBytes / (timeDelta / 1000f)) / 1024;
