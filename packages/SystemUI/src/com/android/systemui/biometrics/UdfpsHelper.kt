...
class UdfpsHelper(
    ...
    private val brightnessAlphaMap: Map<Int, Int> = context.resources
        ...
        .associate {
            val (brightness, alpha) = it.split(",").map { value -> value.trim() }
            brightness to alpha
        }
    ...
    private fun getAlphaForBrightness() {
        ...
        val currentAlpha = brightnessAlphaMap[normalizedBrightness]?.toFloat()?.div(255.0f)
            ?: interpolateAlpha(normalizedBrightness)

        Log.e(TAG, "Brightness: $normalizedBrightness, Alpha: $currentAlpha")
