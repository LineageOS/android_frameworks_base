...
final public class SettingsService extends Binder {
    ...
    final static class MyShellCommand extends ShellCommand {
        ...
        private String getCallMethod(String callMethod) {
            ...
            try {
                Field field;
                if (mUseLineageSettingsProvider) {
                    field = LineageSettings.class.getField(callMethod);
                } else {
                    field = Settings.class.getField(callMethod);
                }
        ...
        public int onCommand(String cmd) {
            ...
            try {
                holder = ActivityManager.getService()
                        .getContentProviderExternal(getSettingsAuthority(),
        ...
        String getForUser(IContentProvider provider, int userHandle,
            ...
            if ("system".equals(table)) callGetCommand = getCallMethod("CALL_METHOD_GET_SYSTEM");
            else if ("secure".equals(table)) callGetCommand = getCallMethod("CALL_METHOD_GET_SECURE");
            else if ("global".equals(table)) callGetCommand = getCallMethod("CALL_METHOD_GET_GLOBAL");
            else {
        ...
        void putForUser(IContentProvider provider, int userHandle, final String table,
            ...
            } else if ("secure".equals(table)) callPutCommand = getCallMethod("CALL_METHOD_PUT_SECURE");
            else if ("global".equals(table)) callPutCommand = getCallMethod("CALL_METHOD_PUT_GLOBAL");
            else {
                getErrPrintWriter().println("Invalid table; no put performed");
                return;
            }
        ...
        void resetForUser(IContentProvider provider, int userHandle,
            ...
            if ("secure".equals(table)) callResetCommand = getCallMethod("CALL_METHOD_RESET_SECURE");
            else if ("global".equals(table)) callResetCommand = getCallMethod("CALL_METHOD_RESET_GLOBAL");
            else {
            ...
            try {
                ...
                arg.putInt(getCallMethod("CALL_METHOD_USER_KEY"), userHandle);
                provider.call(packageName, null, getSettingsAuthority(), callResetCommand, null, arg);
            } catch (RemoteException e) {
                throw new RuntimeException("Failed in IPC", e);
            }
        }
