...
static const char* kLINEAGESDKAssets = "framework/org.lineageos.platform-res.apk";
...
bool AssetManager::addDefaultAssets()
{
    ...
    if (ret) {
        ...
        pathLineage.appendPath(kLINEAGESDKAssets);

        if (!addAssetPath(pathLineage, NULL, false /* appAsLib */, false /* isSystemAsset */)) {
            ALOGE("Failed to load LINEAGESDK resources!");
        }
    }
    return ret;
}
